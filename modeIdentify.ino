...
void identify(bool verbose) {
  ...
  for (int i=0; i < count; i++) {
    ...
    char name[10];
    int sz = tokenise((__FlashStringHelper *)chips[i], ':', name, sizeof(name));
    if (sz == -1) {
    ...
    char desc[100];
    sz = tokenise((__FlashStringHelper *)NULL, ':', desc, sizeof(desc));
    if (sz == -1) {
    ...
    while (true) {
        ...
        sz = tokenise((__FlashStringHelper *)NULL, ':', test, sizeof(test));
        if (sz == -1) {
            if (verbose) {
        ...
        char testdesc[100];
        sz = tokenise((__FlashStringHelper *)NULL, ':', testdesc, sizeof(testdesc));
        if (sz == -1) {
